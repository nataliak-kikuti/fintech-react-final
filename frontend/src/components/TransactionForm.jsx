...
function TransactionForm({ type, initialData, onSave, onCancel }) {
  const { addTransaction, updateTransaction, categories, loading } =
    useFinance()

  const typeUpper = type.toUpperCase()
  const formTitle = type.charAt(0).toUpperCase() + type.slice(1)
  const isEditing = !!initialData

  const filteredCategories = categories.filter(
    cat => cat.tipoCategoria?.toUpperCase() === typeUpper
  )
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setData(
        (initialData.data || new Date().toISOString().split('T')[0]).split(
          'T'
        )[0]
      )
    }
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (type.toLowerCase() === 'receita') {
        transactionData.dataRecebimento = data
      } else if (type.toLowerCase() === 'despesa') {
        transactionData.dataGasto = data
      }
    ...
    <form onSubmit={handleSubmit} className="transaction-form-content">
      ...
      <div className="form-group">
        ...
        {/* <input
          type="text"
          required
          placeholder={type === 'despesa' ? 'Ex: Conta de Luz' : 'Ex: Salário Mensal'}
          value={descricao}
      ...
      <div className="form-group">
        <label>
          {type === 'despesa' ? 'Data do Gasto' : 'Data do Recebimento'}
        </label>
        <input
          type="date"
          required
